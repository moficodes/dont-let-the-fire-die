...
export default function Home() {
  ...
  const hasQuestList = homeData.questList && homeData.questList.length > 0;
  const hasMostWanted = homeData.mostWanted && homeData.mostWanted.length > 0;
  const hasNoticeBoard = hasQuestList || hasMostWanted;
    ...
    <div className="flex flex-col gap-12 pb-24">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-12 text-center w-full">
        <h1 className="text-5xl md:text-6xl font-bold text-primary mb-4 tracking-tight">
          {homeData.header?.title || "Sablewood Chronicles"}
        </h1>
        <p className="text-xl text-on-surface max-w-2xl mx-auto opacity-80">
          {homeData.header?.description || "Welcome to the living record of our adventures, heroes, and the world we shape together."}
        </p>
      </section>
